...
describe('triggers.deal_updated', () => {
  it('should pass webhook payload through perform', async () => {
    ...
    const results = await appTester(
      App.triggers['deal_updated'].operation.perform,
      bundle,
    );
  ...
  it('should subscribe and return an id', async () => {
    ...
    const result = await appTester(
      App.triggers['deal_updated'].operation.performSubscribe,
      bundle,
    );
  ...
  it('should unsubscribe', async () => {
    const bundle = {
      subscribeData: { id: '7462779712490717185' },
      authData: { access_token: process.env.authData_access_token },
      inputData: {},
    };
    const result = await appTester(
      App.triggers['deal_updated'].operation.performUnsubscribe,
      bundle,
    );
